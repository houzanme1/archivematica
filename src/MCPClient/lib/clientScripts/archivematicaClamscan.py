...
class ClamdScanner(ScannerBase):
    ...
    @property
    def version(self):
        return self._version_attrs()[0]

    @property
    def virus_definitions(self):
        return self._version_attrs()[1]

    def _version_attrs(self):
        try:
            self._version
        except AttributeError:
            self._version = self.client.version()
        return clamav_version_parts(self._version)
...
class ClamScanner(ScannerBase):
    ...
    def _version_attrs(self):
        try:
            return clamav_version_parts(self._call('-V'))
        except subprocess.CalledProcessError:
            return None, None

    def scan(self, path):
        passed, state, details = (True, 'OK', None)
        try:
            self._call(path)
